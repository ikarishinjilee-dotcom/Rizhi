			...
			<div class="admin-content">
				...
				<section v-else class="users-panel">
					<div class="panel-heading">
						...
						<RButton variant="secondary" :loading="loading" @click="loadUsers">刷新</RButton>
					</div>
					...
					<div v-else class="user-list">
						<article v-for="user in users" :key="user.id" class="user-row">
							<div class="avatar"><img v-if="user.avatar" :src="user.avatar" alt="" /><template
									v-else>{{ (user.nickname || user.username || "U").slice(0, 1).toUpperCase() }}</template>
							</div>
							<div><strong>{{ user.nickname || user.username }}</strong><small>@{{ user.username }} ·
									{{ user.id }}</small></div>
							<div class="user-badges">
								<span class="role-tag"
									:class="{ admin: user.roles.includes('admin') || user.roles.includes('super_admin') }">{{ roleLabel(user) }}</span>
								<span class="status-tag"
									:class="{ disabled: user.status === 1 }">{{ user.status === 1 ? "已停用" : "正常" }}</span>
							</div>
							<div class="user-actions">
								<RButton v-if="isSuperAdmin() && !user.roles.includes('super_admin')"
									variant="secondary" :disabled="savingUserId === user.id"
									@click="requestRoleChange(user)">
									{{ user.roles.includes("admin") ? "取消管理员" : "设为管理员" }}</RButton>
								<RButton v-if="canChangeStatus(user)"
									:variant="user.status === 1 ? 'secondary' : 'danger'"
									:disabled="savingUserId === user.id" @click="requestStatusChange(user)">
...
<script setup lang="ts">
	...
	const activeTab = ref<"asset" | "account" | "bank" | "branding" | "users">("asset");
