        ...
        <div v-if="notificationOpen" class="notification-panel">
          <div class="notification-panel__head">
            ...
            <div class="notification-panel__head-actions">
              <button type="button" aria-label="提醒设置" :class="{ active: notificationSettingsOpen }" @click="toggleNotificationSettings">
                <Settings2 :size="16" />
              </button>
              <button type="button" aria-label="关闭通知中心" @click="notificationOpen = false"><X :size="16" /></button>
            </div>
          </div>
          <div v-if="notificationSettingsOpen" class="notification-settings">
            ...
            <label>
              <span>提前过保提醒</span>
              <div><RInput v-model="notificationDraft.warrantyDays" /><em>天</em></div>
            </label>
            <label>
              <span>提前还款提醒</span>
              <div><RInput v-model="notificationDraft.repaymentDays" /><em>天</em></div>
            </label>
            <label>
              <span>闲置提醒</span>
              <div><RInput v-model="notificationDraft.idleDays" /><em>天</em></div>
            </label>
            <p v-if="notificationSettingsMessage" class="notification-settings__message" :class="{ danger: notificationSettingsTone === 'danger' }">
            ...
            <div class="notification-settings__actions">
              ...
              <button v-if="ignoredNotificationIds.size" type="button" @click="restoreIgnoredNotifications">
          ...
          <template v-else>
            <div class="notification-toolbar">
              <div>
                <button type="button" :class="{ active: notificationView === 'unread' }" @click="notificationView = 'unread'">
                  未读 {{ unreadNotificationCount }}
                </button>
                <button type="button" :class="{ active: notificationView === 'all' }" @click="notificationView = 'all'">
                  全部 {{ notifications.length }}
                </button>
              </div>
              <button v-if="unreadNotificationCount" type="button" @click="markAllNotificationsRead">全部已读</button>
            </div>
          <div v-if="visibleNotifications.length" class="notification-list">
            <div
              ...
              :class="{ read: readNotificationIds.has(item.id) }"
              role="button"
              tabindex="0"
            ...
            >
              ...
              <button class="notification-item__ignore" type="button" aria-label="忽略这条提醒" @click.stop="ignoreNotification(item)">
                <EyeOff :size="14" />
              </button>
              <ChevronRight :size="15" />
            </div>
...
.topbar::after {
  ...
  top: 0;
  right: 0;
  width: max(0px, calc((100vw - 1592px) / 2));
  height: 56px;
  content: "";
  background: #fff;
  pointer-events: none;
